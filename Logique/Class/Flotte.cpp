...
void Flotte::removeNavire(Navire* navire){
    ...
    if(this->navires->at(index) != navire) error("navire a supprimer non trouve | removeNavire - Flotte");
    if(this->navires->at(index) != NULL) 
    {
    ...
    this->navires->erase(this->navires->begin() + index);
}
